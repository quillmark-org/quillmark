...
/// no content text. Returns `(corpus_chars, byte_len)` for the cluster at byte
...
pub fn emit_richtext(rt: &Content) -> Result<Emission, EmitError> {
    let max_depth = rt
        .lines
        .iter()
...
pub fn emit_richtext_inline(rt: &Content) -> Result<Emission, EmitError> {
    if !rt.is_inline() {
...
mod tests {
    ...
    /// Record, per content char, the `(corpus_index, gen_byte_start)` of the
    ...
    /// content char, and `forward_corpus_offset` round-trips a cluster's content
        ...
        for (ctx, run) in cases {
            ...
            for w in 0..starts.len() {
                ...
                assert_eq!(
                    ...
                    byte_start,
                    "content {corpus_i} of {run:?} ({ctx:?}) → byte {byte_start}"
                );
            }
