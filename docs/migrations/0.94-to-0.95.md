...
## A schema-bound read view — `quill.view(doc)` (#978)

Additive, no action required. The markdown projection was a *schema-shaped*
question ("this field's richtext, as markdown") answered by a schema-free
`Document` — the residue of which was #968's conflation. The projecting read now
has a schema-bound home, the read twin of `quill.writer(doc)`:

```
quill.view(doc)
  .get(addr)   // richtext → markdown, every other type → canonical value
               // absent   → undefined / None
               // unknown field name → throws UnknownField
               // richtext holding an undecodable value → throws FieldRichtextDecode
```
...
(`Markdown(String)` for a richtext field, `Value(QuillValue)` otherwise);
`view.card(i)` is the card cursor. The body read stays quill-free —
`view.getBody()` (WASM/Python) mirrors `getMarkdown()`-of-body, since a body's
type is a format fact, not a schema fact.

`Document.getMarkdown`'s **field** projection is thereby superseded by
`view.get` and will retire in a later release; it still works this release. Its
**body** projection is not superseded and stays on `Document`. Migrating a
field-projection call site is a quill-in-hand rewrite:

```
doc.getMarkdown('subject')          // was
