...
Ships **two artifacts from one crate**, gated by a `render` cargo feature: a Typst-less **core** (`@quillmark/wasm/core`) with `Document` + `Quill` for load / validate / schema / seed / blueprint, and a Typst-backed **render** superset (the root `@quillmark/wasm` import) that adds the `Quillmark` engine + `RenderSession` + canvas. Core is ~0.66 MB gzip; render ~8 MB (Typst dominates). The two modules have separate linear memories, so values cross as serialized data (the quill tree + `doc.toJson()`), not shared handles. See [the split proposal](../proposals/wasm-bindings-split.md).
